...
export default function Home() {
  ...
  const fetchFlights = async () => {
    ...
    try {
      ...
      if (data.data && Array.isArray(data.data)) {
        ...
        const transformedFlights: Flight[] = data.data.map((offer: any, index: number) => {
          ...
          const airlineName = carriers[carrierCode] || carrierCode || 'Unknown'
  ...
  const graphData = useMemo(() => {
    ...
    // Create entries for each airline with their average price
    airlines.forEach((airline) => {
      const avgPrice = airlinePrices[airline].reduce((a, b) => a + b, 0) / airlinePrices[airline].length
      data.push({
        name: airline,
    ...
    // If we have multiple flights, show them as separate data points
    if (filteredFlights.length > airlines.length) {
      return filteredFlights.slice(0, 10).map((flight, index) => ({
        name: `${flight.airline} ${index + 1}`,
        Price: flight.price,
      }))
    }
            ...
            <Card className="sticky top-4 shadow-lg">
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  <Slider
                    value={priceRange}
                    onValueChange={(value) => setPriceRange([priceRange[0], value[1]])}
                    min={0}
                    max={2000}
                    step={10}
                  />
              ...
              <CardContent>
                {loading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">No flights found. Try adjusting your filters.</p>
                  </div>
                ) : (
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
                                ...
                                {flight.aircraft && (
                                  <>
                                    <span>•</span>
                                    <span className="text-xs">{flight.aircraft}</span>
                                  </>
                                )}
                              </div>
                            </div>
